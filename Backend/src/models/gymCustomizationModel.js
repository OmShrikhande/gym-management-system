...
// Static method to get customization for gym members/trainers
gymCustomizationSchema.statics.getForGymUser = function(userId) {
  return this.findOne({ 
    $or: [
      { gymId: userId }, // User is gym owner
      { 'gym.members': userId }, // User is member
      { 'gym.trainers': userId } // User is trainer
    ]
  }).populate('gym', 'name email');
};
...
// Instance method to sanitize and validate custom CSS
gymCustomizationSchema.methods.sanitizeCustomCss = function() {
  if (!this.settings.customCss) return '';
  
  // Basic CSS sanitization - remove potentially dangerous content
  let sanitized = this.settings.customCss
...
export default mongoose.model('GymCustomization', gymCustomizationSchema);